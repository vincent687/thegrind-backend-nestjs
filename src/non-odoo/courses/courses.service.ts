...
export class CoursesService {
  ...
  async create(createCourseDto: CreateCourseDto) {
    ...
    const tutorArray = await createCourseDto.tutors.reduce((acc, val) => {
      return acc.concat({
        cid: newCourse.id,
        user_id: val,
      });
    }, []);
    const studentArray = await createCourseDto.students.reduce((acc, val) => {
      return acc.concat({
        cid: newCourse.id,
        user_id: val,
      });
    }, []);
    const tagArray = await createCourseDto.courseTags.reduce((acc, val) => {
      return acc.concat({
        cid: newCourse.id,
        tag_id: val,
      });
